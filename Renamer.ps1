...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "ContactDetailsApi") {
        $contents -replace 'ContactDetailsApi', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }

    if ($fileName -match 'ContactDetailsApi') {
        $newName = $_.Name -replace 'ContactDetailsApi', $apiName
...
Where-Object { $_.Name -match 'ContactDetailsApi' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'ContactDetailsApi', $apiName
